  ...
  const exportToExcel = () => {
    const exportData = contractions
      ...
      .map((contraction, index) => ({
        ...
        "End Time": contraction.endTime
          ? format(new Date(contraction.endTime), "yyyy-MM-dd HH:mm:ss")
          : "",
        "Duration (seconds)": contraction.duration,
        "Duration (mm:ss)": contraction.duration
          ? `${Math.floor(contraction.duration / 60)}:${(
    ...
    const dataWithIntervals = exportData.map((row, index) => {
      if (index > 0) {
        const currentContraction = contractions.filter(
          (c) => c.duration !== null
        )[index];
        const previousContraction = contractions.filter(
          (c) => c.duration !== null
        )[index - 1];
        const intervalMinutes =
          (new Date(currentContraction.startTime).getTime() -
            new Date(previousContraction.startTime).getTime()) /
          (1000 * 60);
        return {
          ...row,
    ...
    const fileName = `contractions_${format(
      new Date(),
      "yyyy-MM-dd_HH-mm"
    )}.xlsx`;
