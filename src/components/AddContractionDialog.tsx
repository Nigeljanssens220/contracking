...
export function AddContractionDialog({
  open,
  onClose,
  onAdd,
}: AddContractionDialogProps) {
    ...
    if (includeEndTime) {
      if (!endTime) {
        toast({
          title: "Invalid Input",
          description:
            "Please enter an end time or uncheck 'Include end time'.",
          variant: "destructive",
        });
        return;
      }
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter className="gap-2 sm:gap-0">
          ...
          <Button onClick={handleSave}>Add Contraction</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
